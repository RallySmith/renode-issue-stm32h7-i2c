// Derived from 1.14.0 STM32F7_I2C.cs
//
// - Updated to avoid ISR re-entrancy when disabling events
// - Updated to allow byte data transfers
// - Updated to add DMA support (incomplete)
//
...
namespace Antmicro.Renode.Peripherals.I2C
{
    ...
    public sealed class STM32F7_I2C_Fixed : SimpleContainer<II2CPeripheral>, II2CPeripheral, IDoubleWordPeripheral, IKnownSize
    {
        public STM32F7_I2C_Fixed(Machine machine) : base(machine)
        {
        ...
        {
            this.Log(LogLevel.Debug, "Write: data.Length {0}", data.Length);
        ...
        {
            this.Log(LogLevel.Debug, "Read: count {0} : addressMatched {1}", count, addressMatched.Value);
            ...
            {
                this.Log(LogLevel.Debug, "Read: partial");
        ...
        private DoubleWordRegisterCollection CreateRegisters()
        {
            var map = new Dictionary<long, DoubleWordRegister> { {
                    (long)Registers.Control1, new DoubleWordRegister(this)
                        ...
                        .WithFlag(1, out transferInterruptEnabled, changeCallback: InterruptEnableChange, name: "TXIE")
                        .WithFlag(2, out receiveInterruptEnabled, changeCallback: InterruptEnableChange, name: "RXIE")
                        .WithFlag(3, out addressMatchedInterruptEnabled, changeCallback: InterruptEnableChange, name: "ADDRIE")
                        .WithFlag(4, out nackReceivedInterruptEnabled, changeCallback: InterruptEnableChange, name: "NACKIE")
                        .WithFlag(5, out stopDetectionInterruptEnabled, changeCallback: InterruptEnableChange, name: "STOPIE")
                        .WithFlag(6, out transferCompleteInterruptEnabled, changeCallback: InterruptEnableChange, name: "TCIE")
                        .WithFlag(7, out errorInterruptEnabled, changeCallback: InterruptEnableChange, name: "ERRIE")
                        .WithTag("DNF", 8, 4)
                        .WithTag("ANFOFF", 12, 1)
                        .WithReservedBits(13, 1)
                        .WithFlag(14, writeCallback: DMAEnableTXWrite, name: "TXDMAEN")
                        .WithFlag(15, writeCallback: DMAEnableRXWrite, name: "RXDMAEN")
                        .WithTag("SBC", 16, 1)
        ...
        private void InterruptEnableChange(bool oldValue, bool newValue)
        {
            ...
            else
            {
                ...
                // re-entrancy when CR1 written to disable source:
                Update();
            }
        }
        ...
        private void PeripheralEnabledWrite(bool oldValue, bool newValue)
        {
            ...
            this.Log(LogLevel.Debug, "PeripheralEnableWrite: oldValue {0} newValue {1}", oldValue, newValue);
            if(newValue)
            {
                return;
            }
        ...
        private void DMAEnableTXWrite(bool oldValue, bool newValue)
        {
            this.Log(LogLevel.Debug, "DMAEnableTXWrite: oldValue {0} newValue {1}", oldValue, newValue);
            if(newValue)
            {
            ...
            this.Log(LogLevel.Debug, "DMAEnableTXWrite: DMATransmit {0}", DMATransmit.IsSet);
        }

        private void DMAEnableRXWrite(bool oldValue, bool newValue)
        {
            this.Log(LogLevel.Debug, "DMAEnableRXWrite: oldValue {0} newValue {1}", oldValue, newValue);
            if(newValue)
            {
                if (!DMAReceive.IsSet)
                {
            ...
            this.Log(LogLevel.Debug, "DMAEnableRXWrite: DMAReceive {0}", DMAReceive.IsSet);
        }
        ...
        private void StartWrite(bool oldValue, bool newValue)
        {
            ...
            this.Log(LogLevel.Debug, "StartWrite: oldValue {0} newValue {1}", oldValue, newValue);
            if(!newValue)
            {
                return;
            }
        ...
        {
            this.Log(LogLevel.Debug, "StopWrite: oldValue {0} newValue {1}", oldValue, newValue);
            if(!newValue)
            {
                return;
            }
        ...
        {
            this.Log(LogLevel.Debug, "ReceiveDataRead: oldValue {0}", oldValue);
            if(rxData.Count > 0)
            {
        ...
        {
            this.Log(LogLevel.Debug, "HandleTransmitDataWrite: oldValue {0} newValue {1}", oldValue, newValue);
            if(masterMode)
            {
        ...
        {
            this.Log(LogLevel.Debug, "MasterTransmitDataWrite: oldValue {0} newValue {1}", oldValue, newValue);
        ...
        {
            this.Log(LogLevel.Debug, "SlaveTransmitDataWrite: oldValue {0} newValue {1}", oldValue, newValue);
            txData.Enqueue((byte)newValue);
        }

        private void SetTransferCompleteFlags()
        {
            this.Log(LogLevel.Debug, "SetTransferCompleteFlags");
        ...
        private void Update()
        {
            this.Log(LogLevel.Debug, "Update");
            ...
            // TODO: implement: ARLO | BERR | OVR | TIMEOUT | PECERR | ALERT
            ErrorInterrupt.Set(errorInterruptEnabled.Value && false);
        }
